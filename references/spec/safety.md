...
- **Graceful Integration Degradation**: Sensors depending on external APIs or unreliable integrations must degrade gracefully. Use safe defaults, loose availability gates (only require truly critical inputs), expose degradation state (`data_quality`, `reasoning`), and ensure downstream automations check degradation status before proceeding. See `references/patterns/integration_degradation.md`.
